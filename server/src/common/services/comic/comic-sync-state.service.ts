...
export class ComicSyncStateService {
    ...
    private data: Record<string, SyncStateData> = {}
    ...
    set(book_id: string, state: SyncStateData) {
        this.data[book_id] = state
        this.stateOverview$.next(this.data)
    }

    update_state(book_id: string, state: SyncStateData['state']) {
        this.data[book_id].state = state
        this.send_overview()
        this.send_book_state(book_id)
    }

    update_book(book_id: string, progress: number) {
    ...
    subscribe_book(book_id: string, callback: (value: SyncStateData) => void) {
    ...
    private send_book_state(book_id: string) {
        if (this.state_map[book_id]) {
